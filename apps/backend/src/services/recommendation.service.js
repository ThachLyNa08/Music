...
async function fetchAudioFeaturesForSongs(songIds) {
  ...
  const [rows] = await pool.query(`
    SELECT song_id,
           ...
           ${columns.loudness ? 'loudness' : 'NULL'} AS loudness,
           ${columns.dynamic_complexity ? 'dynamic_complexity' : 'NULL'} AS dynamic_complexity,
           ${columns.study_suitability_score ? 'study_suitability_score' : 'NULL'} AS study_suitability_score,
           ${columns.calm_fit_score ? 'calm_fit_score' : 'NULL'} AS calm_fit_score,
           ${columns.analysis_version ? 'analysis_version' : 'NULL'} AS analysis_version,
           ${columns.analysis_updated_at ? 'analysis_updated_at' : 'NULL'} AS analysis_updated_at,
