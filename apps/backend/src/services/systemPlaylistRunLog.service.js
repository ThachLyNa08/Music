...
async function ensureGenerationRunsTableExists() {
  ...
  await addColumnIfMissing('system_playlist_generation_runs', 'total_playlists', 'total_playlists INT DEFAULT 0 AFTER total_users');
  await addColumnIfMissing('system_playlist_generation_runs', 'success_count', 'success_count INT DEFAULT 0 AFTER total_playlists');
    ...
    try {
      ...
      const [result] = await conn.query(
        `INSERT INTO system_playlist_generation_runs
          (operation_type, status, started_at, heartbeat_at, total_users, total_playlists, triggered_by_user_id, metadata)
         VALUES (?, ?, NOW(), NOW(), ?, ?, ?, ?)`,
        [
          operationType,
          initialStatus,
