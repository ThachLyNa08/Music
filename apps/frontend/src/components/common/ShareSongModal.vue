      ...
      <div class="w-full max-w-md overflow-hidden rounded-lg border border-white/10 bg-[#090d18] shadow-2xl">
        ...
        <div class="p-5">
          <div class="flex items-center gap-3 rounded-lg border border-white/10 bg-white/[0.04] p-3">
            <img :src="songCover" alt="" class="h-14 w-14 rounded-md object-cover" @error="event => event.target.src = '/default-cover.png'" />
            <div class="min-w-0">
              <div class="truncate text-sm font-extrabold text-white">{{ songTitle }}</div>
              <div class="truncate text-xs font-semibold text-slate-500">{{ songArtist }}</div>
            </div>
          </div>
...
const songTitle = computed(() => props.song?.title || props.song?.name || 'Bài hát')
const songArtist = computed(() => props.song?.artist_name || props.song?.artist || props.song?.artists || 'Nghệ sĩ')
const songCover = computed(() => normalizeImageUrl(props.song?.cover_url || props.song?.cover || props.song?.image_url || ''))
...
async function sendTo(item) {
  if (!songId.value || sending.value) return
  sending.value = true
  try {
    let response
    if (item.type === 'conversation') {
      response = await messagesApi.shareSongToConversation(item.conversationId, songId.value)
      sentConversationId.value = item.conversationId
    } else {
      response = await messagesApi.shareSongToUser(item.user.id, songId.value)
      sentConversationId.value = response.data.data?.conversationId
    }
    toast.showToast(`Đã chia sẻ bài hát với ${item.user.display_name}`, 'success')
  } finally {
    sending.value = false
  }
}
