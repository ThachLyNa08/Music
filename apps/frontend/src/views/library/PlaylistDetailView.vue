      ...
      <div class="relative z-10 flex flex-col lg:flex-row items-center lg:items-center gap-6 md:gap-8 max-w-[1400px] mx-auto">
        ...
        <div class="flex flex-col gap-1.5 min-w-0 flex-1 text-center lg:text-left w-full">
          ...
          <div class="playlist-actions mt-3 flex flex-wrap items-center justify-center lg:justify-start gap-3">
            ...
            <button v-if="canDeletePlaylist && !canEditMetadata" class="w-11 h-11 rounded-full bg-red-500/10 backdrop-blur-md border border-red-500/30 text-red-400 flex items-center justify-center hover:bg-red-500 hover:text-white hover:scale-105 transition-all shadow-lg cursor-pointer" @click="deletePlaylist" title="Xóa playlist">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="18" height="18"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
            </button>
...
const canDeletePlaylist = computed(() => {
  if (!playlist.value || !isOwner.value || isSystemPlaylist.value) return false
  if (playlist.value.can_delete != null) return Boolean(playlist.value.can_delete)
  return isManualPlaylist.value || isAiPlaylist.value
})
...
function deletePlaylist() {
  if (!canDeletePlaylist.value) return;
  openConfirm({
    title: 'Xóa playlist?',
