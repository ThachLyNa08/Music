<template>
  <div class="user-layout-surface pb-4">
    ...
    <div v-else-if="song" class="w-full">
      ...
      <div class="space-y-8 px-4 md:px-8 py-4 max-w-[1400px] mx-auto">
        ...
        <section class="song-detail-card h-fit">
          <div class="mb-4 flex items-center justify-between gap-3">
            <h2 class="text-lg font-extrabold text-white flex items-center gap-2">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20" class="text-emerald-400">
            ...
            <span v-if="lyricsBadge" class="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-[11px] font-extrabold uppercase tracking-wide text-slate-300">
              {{ lyricsBadge }}
            </span>
          </div>

          <div v-if="lyricsLoading" class="py-8 text-center text-sm font-semibold text-slate-400">
            Đang tải lời bài hát...
          </div>
          
          <div v-else-if="lyricsError" class="lyrics-empty">
            Không thể tải lời bài hát
          </div>

          <div v-else-if="lyricsEmptyMessage" class="lyrics-empty">
            {{ lyricsEmptyMessage }}
          </div>
          ...
          <div v-else class="relative">
            <div
              class="lyrics-reader"
              :class="{ 'max-h-72 overflow-hidden': !showFullLyrics && isLyricsLong }"
            >
              <p v-for="(line, index) in displayedLyricsLines" :key="`${index}-${line}`" class="lyrics-reader-line">
                {{ line }}
              </p>
            </div>
            <div v-if="isLyricsLong && !showFullLyrics" class="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#111827] to-transparent pointer-events-none"></div>
            <button 
              v-if="isLyricsLong"
              @click="showFullLyrics = !showFullLyrics"
              class="mt-4 text-emerald-400 hover:text-emerald-300 text-sm font-bold flex items-center gap-1 transition"
            >
        ...
        <section class="song-detail-card h-fit">
          <h2 class="text-lg font-extrabold text-white mb-4 flex items-center gap-2">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="18" height="18" class="text-emerald-400">
      ...
      <!-- 3. POPULAR SONGS -->
      <section v-if="popularSongs.length > 0" class="song-detail-card">
      ...
      <!-- 4. ALBUMS -->
      <section v-if="artistAlbums.length > 0" class="song-detail-card">
        <h2 class="user-section-title text-lg mb-4 flex items-center gap-2">
      ...
      <!-- 5. RELATED SONGS -->
      <section v-if="relatedSongs.length > 0" class="song-detail-card">
...
const lyricsSyncType = computed(() => lyricsData.value?.syncType || 'NONE')

const displayedLyricsLines = computed(() => {
  const lines = Array.isArray(lyricsData.value?.lines) ? lyricsData.value.lines : []
  if (lines.length) {
    return lines
      .map((line) => String(line?.words || '').trim())
      .filter(Boolean)
  }

  const plainLyrics = lyricsData.value?.plainLyrics || lyricsData.value?.plain_lyrics || ''
  if (plainLyrics) {
  ...
  const syncedLyrics = lyricsData.value?.syncedLyrics || lyricsData.value?.synced_lyrics || ''
  if (syncedLyrics) {
    return cleanLyrics(String(syncedLyrics))
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean)
  }
...
const lyricsBadge = computed(() => {
  if (!lyricsData.value || lyricsData.value?.error || lyricsSyncType.value === 'NONE') return ''
  if (lyricsSyncType.value === 'LINE_SYNCED') return lyricsData.value.provider ? `${lyricsData.value.provider} • Synced` : 'Synced'
  if (lyricsSyncType.value === 'PLAIN_TEXT') return lyricsData.value.provider ? `${lyricsData.value.provider} • Plain` : 'Plain'
  if (lyricsSyncType.value === 'INSTRUMENTAL') return 'Instrumental'
  return lyricsData.value.provider || ''
})

const lyricsEmptyMessage = computed(() => {
  if (lyricsSyncType.value === 'INSTRUMENTAL') return 'Bài hát không lời'
  if (lyricsData.value?.error || lyricsSyncType.value === 'NONE') return 'Chưa có lời bài hát cho bài này'
  if (!displayedLyricsLines.value.length) return 'Chưa có lời bài hát cho bài này'
  return ''
})

const isLyricsLong = computed(() => {
  return displayedLyricsLines.value.join('\n').length > 700 || displayedLyricsLines.value.length > 14
})
...
async function fetchSongLyrics(songId) {
  ...
  try {
    const response = await getLyricsBySongId(songId)
    ...
    if (token === lyricsRequestToken) lyricsLoading.value = false
  }
}
...
function openAddToPlaylist() {
  ...
  library.openPlaylistModal(song.value)
}
