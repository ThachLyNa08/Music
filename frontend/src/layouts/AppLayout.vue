<template>
  <div class="user-layout-surface min-h-screen w-full overflow-x-hidden text-gray-200 font-sans">
    ...
    >
      ...
      <!-- Page content -->
      <main class="relative z-10 flex-1 px-4 pt-24 pb-[112px] md:px-8">
        <RouterView />
      </main>
    </div>
        ...
        <div>
          ...
          <div class="flex flex-col gap-1.5" v-else>
            <div 
              ...
              :key="song.id + '-' + idx" 
              class="user-row flex items-center gap-3 p-2.5 cursor-pointer group border border-transparent hover:border-white/10"
    ...
    <footer class="fixed bottom-0 left-0 right-0 z-[999] flex h-[96px] items-center justify-between border-t border-white/10 bg-[#05070d]/95 px-4 shadow-[0_-18px_45px_rgba(0,0,0,0.45)] backdrop-blur-xl">
      ...
      <div class="flex items-center gap-1 flex-1 justify-end">
        ...
        <button class="bg-transparent border-none cursor-pointer p-1.5 rounded-full transition-all duration-200 text-gray-500 hover:text-white" title="Karaoke">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4">
...
const navItems = [
  ...
  { to: '/karaoke', label: 'Karaoke AI', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z"/></svg>' },
]
</script>
