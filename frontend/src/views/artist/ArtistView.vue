<template>
  <div class="artist-page relative user-page-bg pb-4" v-if="artist" :style="{ '--artist-color': dominantColor }">
    ...
    <div class="sticky-header" :class="{ 'visible': showStickyHeader }">
      <div class="flex items-center gap-4 px-6 h-full">
        <button class="sticky-play" @click="playArtist" title="Phát">
    ...
    <div class="content-wrapper">
      ...
      <div class="relative z-10 px-6 md:px-8 py-6">
        ...
        <section class="action-bar flex items-center gap-6 mb-10">
          <button class="play-btn-large user-play-btn w-14 h-14" @click="playArtist" title="Phát">
            <svg viewBox="0 0 24 24" class="w-7 h-7 fill-black ml-1"><polygon points="5 3 19 12 5 21 5 3" /></svg>
          </button>
  ...
  <AddToPlaylistModal
    :show="playlistModalState.show"
    :song="playlistModalState.song"
    @close="playlistModalState.show = false"
...
function playArtist() {
  if (!artist.value) return
  const queue = artist.value.songs?.length ? artist.value.songs : artist.value.popular_songs
...
async function handleToggleLike(song) {
  ...
  try {
    const isCurrentlyLiked = song.is_liked === 1
    if (isCurrentlyLiked) {
      ...
      song.is_liked = 1
      showToast('Đã lưu vào Bài hát đã thích')
    }
  } catch (err) {
    showToast('Lỗi cập nhật yêu thích')
  }
}
...
function handleAddToPlaylist(song) {
  ...
  playlistModalState.value = { show: true, song }
}
