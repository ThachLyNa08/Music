...
async function fetchRecommendations() {
  ...
  try {
    const raw = await fetchRecommendationCandidates()
    let picked = pickRecommendedSongs(raw, { avoidCurrent: false })

    if (picked.length === 0) {
      const fallbackRaw = await fetchFallbackSongs()
      picked = pickRecommendedSongs(fallbackRaw, { avoidCurrent: false })
    }
    ...
    const fallbackRaw = await fetchFallbackSongs()
    const picked = pickRecommendedSongs(fallbackRaw, { avoidCurrent: false })
...
async function refreshRecommendations() {
  ...
  const previousSongs = [...recommendations.value]

  try {
    const raw = await fetchRecommendationCandidates()
    let picked = pickRecommendedSongs(raw, { avoidCurrent: true })

    if (picked.length === 0) {
      picked = pickRecommendedSongs(raw, { avoidCurrent: false })
    }

    if (picked.length === 0) {
      const fallbackRaw = await fetchFallbackSongs()
      picked = pickRecommendedSongs(fallbackRaw, { avoidCurrent: true })
    ...
    if (picked.length > 0) {
      recommendations.value = picked.slice(0, RECOMMEND_LIMIT)
    } else {
      recommendations.value = previousSongs
      console.warn('[PlaylistRecommendations] refresh returned empty, keeping previous songs')
    }
  } catch (err) {
